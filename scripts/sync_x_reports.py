...
def write_payload(out_path, username, filter_tag, reports, error=None):
    payload = {
        "source": f"X:@{username}" if username else "X:@unknown",
        "filter": filter_tag,
...
def api_get(path, token, params=None):
    ...
    try:
        ...
        try:
            ...
            pass
        return {"__error__": {"code": e.code, "body": body, "path": path}}

    except URLError as e:
        return {"__error__": {"code": "URLError", "body": str(e), "path": path}}
...
def main():
    ...
    if not token or not username:
        write_payload(out_path, username, filter_tag, [], "Missing X_BEARER_TOKEN or X_USERNAME")
        print("Missing X_BEARER_TOKEN or X_USERNAME. Wrote empty reports.json.", file=sys.stderr)
        return 0
    ...
    if "__error__" in user:
        ...
        code = err.get("code")
        if code == 402:
            msg = "X API 402 Payment Required: token tier has no access to this endpoint. Use a paid X API plan/token."
        else:
            msg = f"X API error while resolving user id: {err}"
        write_payload(out_path, username, filter_tag, [], msg)
        print(msg, file=sys.stderr)
        return 0

    user_id = (user.get("data") or {}).get("id")
    if not user_id:
        msg = "Could not resolve user id"
        write_payload(out_path, username, filter_tag, [], msg)
        print(msg, file=sys.stderr)
        return 0
    ...
    if "__error__" in posts:
        err = posts["__error__"]
        code = err.get("code")
        if code == 402:
            msg = "X API 402 Payment Required: token tier has no access to this endpoint. Use a paid X API plan/token."
        else:
            msg = f"X API error while fetching tweets: {err}"
        write_payload(out_path, username, filter_tag, [], msg)
        print(msg, file=sys.stderr)
        return 0
    ...
    write_payload(out_path, username, filter_tag, reports)
